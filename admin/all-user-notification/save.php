...
if (($_FILES["video"]["type"] == "video/mp4") || ($_FILES["video"]["type"]=="video/ogg") || ($_FILES["video"]["type"] == "video/mov") || ($_FILES["video"]["type"]=="video/3gp") && ($_FILES["video"] ["size"] < 52428800) && in_array($video_extension,  $allowedExts))
{
 ...
 $video = '';
	...
	$sql="insert into tbl_notification(title,type,user_id,image,video,description,status,created,time,date_time)values('$title','user','$userList','$image','$video','$description','$status','$current_date','$time','$date_time')"; 
 ...
 foreach ($useraarray as $newuser) {
  ...
  echo $newUserList = $rown_user['email'];
	...
	$to = $newUserList;
	$subject = 'Blastergate Notification';
                            ...
                            <tr>
                                ...
                                <tr>
                                    <td style="font-size:14px; color:#000; line-height:18px;">
                                        <p style="font-size: 18px;font-weight: bold;">Welcome to BlasteGate</p>
