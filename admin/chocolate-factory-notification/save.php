...
if (($_FILES["video"]["type"] == "video/mp4") || ($_FILES["video"]["type"]=="video/ogg") || ($_FILES["video"]["type"] == "video/mov") || ($_FILES["video"]["type"]=="video/3gp") && ($_FILES["video"] ["size"] < 52428800) && in_array($video_extension,  $allowedExts))
{
 ...
 $video = '';
	...
	$sql="insert into tbl_notification(title,type,user_id,image,video,description,status,created,time,date_time)values('$title','pay_per_click','$userList','$image','$video','$description','$status','$current_date','$time','$date_time')"; 

	$qrs=mysqli_query($connect,$sql);


  // $userdata="select * from tbl_admin";
  // $guser=mysqli_query($connect,$userdata);
  // $rown_smtp=mysqli_fetch_array($guser);

  // $smtp_Host = $rown_smtp['smtp_host'];
  // $smtp_Port = $rown_smtp['smtp_port'];
  // $smtp_Username = $rown_smtp['smtp_user'];
  // $smtp_Password = $rown_smtp['smtp_pass'];
  // print_r($useraarray);
 ...
 foreach ($useraarray as $newuser) {
  ...
  echo $newUserList = $rown_user['email'];
   

	$to = $newUserList;
	$subject = 'Blastergate Chocolate Factory Notification';
                            ...
                            <tr>
                                ...
                                <tr>
                                    <td style="font-size:14px; color:#000; line-height:18px;">
                                        <p style="font-size: 18px;font-weight: bold;">Welcome to BlasteGate Chocolate Factory User</p>
