...
@Component({
  selector: 'app-couple-user-profile',
  templateUrl: './couple-user-profile.component.html',
  styleUrls: ['./couple-user-profile.component.css']
})
export class CoupleUserProfileComponent implements OnInit {
...
public who_i_viewed(){
    ...
    this.callapi.post('/profile/view_validate_request',user_validate_obj).subscribe(result3 => {
       ...
       if(result3.status == 200){
          ...
          if(this.validate_request_status == 'pending'){
            this.validate_request_bg_color = 'bg-blue';
          }else if(this.validate_request_status == 'accepted'){
