...
export class RememberComponent implements OnInit {
  ...
  public getallFriendRequest(bodydata){
    ...
    this.callapi.post('/feed/get_all_remember_request',bodydata).subscribe(data => {
      $('#preloader').hide();
       this.title_heading = 'Whom I Like';
  ...
  //   this.callapi.get('/feed/get_all_send_like_request').subscribe(data => {
  //       this.title_heading = 'Who Like Me';
  //        this.pending_data=false;
  //       if(data.status == 200){
