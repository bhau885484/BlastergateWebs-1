...
@Component({
  selector: 'app-who-i-viewed',
  templateUrl: './who-i-viewed.component.html',
  styleUrls: ['./who-i-viewed.component.css']
})
export class WhoIViewedComponent implements OnInit {
  ...
  public getOnlineUser(bodydata){
     ...
     this.callapi.post('/user/who_i_viewed',bodydata).subscribe(data => {
       $('#preloader').hide();
        if(data.status == 200){
