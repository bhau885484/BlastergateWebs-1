...
export class EventDetailsComponent implements OnInit {
  ...
  public eventFormSingle: FormGroup;
  public eventFormCouple: FormGroup;
  public submitted = false;
  public submittedSingle = false;
  public submittedCouple = false;
  public base64Image1;
  public userData;
  public eventPrice;
  ...
  public guestMemberFlag:boolean=false;
  ...
  ngOnInit() {
    ...
    this.commonServe.promo_code_id = '0';
    ...
    this.getPaymentType();
     ...
     setTimeout(() => {
        ...
        this.eventFormSingle = this.formBuilder.group({
              itemsSingle: this.formBuilder.array([]),
             });

        this.eventFormCouple = this.formBuilder.group({
  ...
  get itemsSingle(): FormArray {
    return this.eventFormSingle.get('itemsSingle') as FormArray;
  }

  get itemsCouple(): FormArray {
    return this.eventFormCouple.get('itemsCouple') as FormArray;
  }

  addItemSingle(): void {
    if (this.itemsSingle.length < 5) {
      this.itemsSingle.push(this.formBuilder.group({
          full_name1: new FormControl('', [Validators.required, Validators.pattern(this.fullnamePattern)]),
  ...
  addItemCouple(): void {
    if (this.itemsCouple.length < 5) {
      this.itemsCouple.push(this.formBuilder.group({
          ...
          full_name2: new FormControl('', [Validators.required, Validators.pattern(this.fullnamePattern)]),
  ...
  public removesecondForm(){
    ...
    this.eventForm = this.formBuilder.group({
      full_name: new FormControl('', [Validators.required, Validators.pattern(this.fullnamePattern)]),
    ...
    this.callapi.post('/events/get_single_events',obj).subscribe(data => {
       if(data.status == 200){
         ...
         this.loaderFlag = true;
          ...
          if(data.room_list.status == 200){
             this.roomData = data.room_list.data;
           }else{
              this.roomData = '';
           }
  ...
  public removePromoCode(){
     $('#add_promo').css('display','block');
     $('#remove_promo').css('display','none');
     $('#promo_code').val('');
     this.commonServe.discount_price = 0;
     this.commonServe.event_total_sum_amount = parseFloat(this.commonServe.event_sub_total_sum_amount) - parseFloat(this.membership_discount_price);
  }
  ...
  public checkPromo(){
    ...
    if(promo_code ==''){
      this.toastr.errorToastr("Please enter a promo code",'Error!');
       ...
       if(this.commonServe.event_sub_total_sum_amount > 0){
            var obj={
          ...
          this.callapi.post('/events/check_promo',obj).subscribe(data => {
            $('#preloader').hide();
             if(data.status == 200)
             {
               ...
               $('#add_promo').css('display','none');
               $('#remove_promo').css('display','block');
               this.commonServe.event_total_sum_amount = parseFloat(this.commonServe.event_sub_total_sum_amount) - parseFloat(this.commonServe.discount_price) - parseFloat(this.membership_discount_price);
         ...
         this.toastr.errorToastr('Please add atleast one guest in room..', 'Error!');
       }
     ...
     if(obj){
       $('#preloader').show();
         this.callapi.get('/membership/check_single_username_membership/' + obj).subscribe(result => {
          $('#preloader').hide();
            if((result.status === '200')) {
              this.toastr.successToastr("This guest is a member of the Blastergate.",'Success!');
              
              const faControl = (<FormArray>this.eventFormSingle.controls['itemsSingle']).at(index);
              ...
              this.toastr.errorToastr("This guest is not the member of the Blastergate",'Error!');
                const faControl = (<FormArray>this.eventFormSingle.controls['itemsSingle']).at(index);
               faControl['controls'].full_name1.setValue('');
         ...
         this.callapi.get('/membership/check_couple_username_membership/' + obj).subscribe(result => {
          $('#preloader').hide();
            if((result.status === '200')) {
              ...
              const faControl1 = (<FormArray>this.eventFormCouple.controls['itemsCouple']).at(index);
              ...
              this.toastr.errorToastr("This guest is not the member of the Blastergate",'Error!');
                const faControl1 = (<FormArray>this.eventFormCouple.controls['itemsSingle']).at(index);
               faControl1['controls'].full_name1.setValue('');
  ...
  public checkJoinMember(event: any){
      ...
      if(isChecked == true)
      {
        ...
        this.commonServe.event_total_sum_amount =  parseFloat(this.commonServe.event_total_sum_amount) + parseFloat(this.membership_discount_price);  
        this.commonServe.event_sub_total_sum_amount = 0;
  ...
  public buyNow(type){
      ...
      const dataSingle = this.eventFormSingle.value; 
      const dataCouple = this.eventFormCouple.value; 

      // console.log('this.eventForm.value',this.eventForm.value);
      if ((this.eventForm.valid) && (this.eventFormSingle.valid) && (this.eventFormCouple.valid)) 
      {
          // alert(this.joinMemeberFlag);
          this.submitted = false;
          this.submittedSingle = false;
          this.submittedCouple = false;

          this.usenameArraySingle=[];
          this.usenameArrayCouple=[];
          this.usenameArray=[];
          this.addRoomUserList=[];
          ...
          dataSingle.itemsSingle.forEach((element:any)=>{
          ...
          dataCouple.itemsCouple.forEach((element:any)=>{
            this.usenameArray.push({"full_name":element.full_name1,"email":element.email1,"id_proof":element.id_proof1,"mobile":element.mobile1,"username":element.username1,"add_new":0,"event_id":this.event_id,"user_id":this.user_id})
          })
         ...
         const hasDuplicates = new Set(username).size !== username.length;
          ...
          this.roomUserArray.sort((a, b) => {
            let nameComparison = a.username.localeCompare(b.username);
            if (nameComparison === 0) {
              return a.mobile - b.mobile; // If names are the same, sort by age
            }
            return nameComparison;
          });

          console.log('addAF',this.usenameArray);
          ...
          if(this.usenameArray.length > 0)
          {
            ...
            this.final_total2 =this.membership_discount_price;
           ...
           this.submitted = true;
           this.submittedSingle = true;
           this.submittedCouple = true;
           return;
      }
  }
 ...
 public showHideFunction1(index){
  ...
  public changeRoomQty(qty,room_id,room_detials){
   ...
   if(this.roomUserArray.length > 0){
      if(qty > 0)
      {
 ...
 public eventTotalSum(qty,id,fee,price){
     ...
     const index1 = this.totalRoomUserAmount.findIndex(user => user.id === id &&  user.price === price && user.fee === fee);
      ...
      if (index1 !== -1) {
        ...
        for (let j = 0; j < this.totalRoomUserAmount.length; j++) {
          ...
          this.commonServe.event_total_sum_amount =  amount - parseFloat(this.final_total2);
          this.commonServe.event_sub_total_sum_amount = amount;
       } 
  ...
  public changePartialPaymentQty(qty,show_partial_payment_details){
    ...
    var sub_total = parseFloat(show_partial_payment_details.payment_fee) +parseFloat(this.down_payment);
    this.down_payment_total = sub_total;

    this.partial_payment_fee = show_partial_payment_details.payment_fee;
  ...
  public paymentBuy() {
    ...
    if(this.usenameArray.length > 0)
    {
      if(this.usenameArray.length == this.addRoomUserList.length){
        ...
        this.toastr.errorToastr("Please add user in room",'Error!');
      }

    }else{
      this.toastr.errorToastr("Please select atleast one user in room",'Error!');
    }
  ...
  public savePayment() {
        ...
        if (this.paymentgetwayForm.valid) {
          ...
          var newobj={
              ...
              "sub_total":this.commonServe.event_sub_total_sum_amount,
              "total_amount":this.commonServe.event_total_sum_amount,
              "partial_payment_amount":this.total_paid_amount,
              "partial_payment_fee":this.partial_payment_fee,
              "final_pay_amount":this.final_total,
              "partial_payment_amount_without_fee":this.down_payment,
